...
#pragma mark * SpaSimplePing

@interface SpaSimplePing ()
...
- (void)didFailWithError:(NSError *)error {
    id<SpaSimplePingDelegate>  strongDelegate;
...
- (void)setTimeout:(long)sec
{
    ...
    setsockopt(CFSocketGetNative(self.socket), SOL_SOCKET, SO_RCVTIMEO, (void *)&tv, sizeof(tv));
}
...
static void SocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    obj = (__bridge SpaSimplePing *) info;
    assert([obj isKindOfClass:[SpaSimplePing class]]);
        ...
        for (NSData * address in addresses) {
            ...
            if ( address.length >= sizeof(struct sockaddr) ) {
                switch (addrPtr->sa_family) {
                    case AF_INET: {
                        if (self.addressStyle != SpaSimplePingAddressStyleICMPv6) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
                    case AF_INET6: {
                        if (self.addressStyle != SpaSimplePingAddressStyleICMPv4) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
