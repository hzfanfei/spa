...
@protocol SpaSimplePingDelegate;

/*! Controls the IP address version used by SpaSimplePing instances.
 */

typedef NS_ENUM(NSInteger, SpaSimplePingAddressStyle) {
    SpaSimplePingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    SpaSimplePingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    SpaSimplePingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
...
@interface SpaSimplePing : NSObject
...
@property (nonatomic, weak, readwrite, nullable) id<SpaSimplePingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) SpaSimplePingAddressStyle addressStyle;
...
/*! A delegate protocol for the SpaSimplePing class.
 */

@protocol SpaSimplePingDelegate <NSObject>

@optional

/*! A SpaSimplePing delegate callback, called once the object has started up.
...
/*! A SpaSimplePing delegate callback, called if the object fails to start up.
 *  \details This is called shortly after you start the object to tell you that the
...
/*! A SpaSimplePing delegate callback, called when the object has successfully sent a ping packet.
...
/*! A SpaSimplePing delegate callback, called when the object fails to send a ping packet.
 *  \details Each call to `-sendPingWithData:` will result in either a
 *      `-simplePing:didSendPacket:sequenceNumber:` delegate callback or a
 *      `-simplePing:didFailToSendPacket:sequenceNumber:error:` delegate callback (unless you
 *      stop the object before you get the callback).  These callbacks are currently delivered
...
/*! A SpaSimplePing delegate callback, called when the object receives a ping response.
...
/*! A SpaSimplePing delegate callback, called when the object receives an unmatched ICMP message.
 ...
 *
 *      This is declared in the header because clients of SpaSimplePing might want to use
